...
\begin{figure*}[!t]
	\begin{center}	
%		\includegraphics[width=.45\textwidth]{decision_quality.pdf}
...
\subsubsection*{Platform}
Our dynamic experiments are all run on an Intel Nehalem-EP system with two 2.66-GHz Xeon X5550 quad-core processors and hyperthreading enable (\emph{i.e.,} 16 hardware threads).
...
Video Application
psearchy
Network Bandwidth Ballon Application
...
%		\includegraphics[width=.45\textwidth]{cluster_decision_points.pdf}
%		\includegraphics[width=.45\textwidth]{parsec_decision_points.pdf}
		\caption{Resource Allocations of video conferencing through time as participants enter and leave and the video in focus changes.}
		\label{video_experiment}
	\end{center}
\end{figure*}

Figure~\ref{video_experiment} shows the resource allocations for our video conference scenario.


\fix{Data: resource allocation through time as applications start and stop}


\fix{Data: Move to Dynamic Optimization, runtime of online resource allocation}
\subsubsection*{RTF Experiments}
\fix{Move to online modeling section}
