...
%Useful citations
%\cite{tess09}, \cite{SPEC2006}, \cite{parsec}, \cite{dacapo}, \cite{lithe}, \cite{matlab}, \cite{cvx}, \cite{pulse}, \cite{bates-aes03}
%PULSE (Preemptive User-Level SchEduling)
%Lithe (LIquid THrEads)

We use two different implementations of the \pacora framework for evaluation. Our initial offline framework collects data online using an x86 processor running Linux.  That data is then used to build models and make and evaluation resource allocation decisions offline.  Our offline framework was used to experiment with the accuracy of different types of models and test the quality of the resource allocation decisions.  Our online framework is implemented in a research operating system, \tess, and is used to test our implementations of the algorithms, measure the overhead and reaction times, and illustrate \pacora's ability to work in a real system.  In this section, we describe each of the implementations and associated experiments in more detail.
...
We use a prototype version of Intel's Sandy Bridge x86 processor that is similar to the
commercially available client chip, but with additional hardware
support for way-based LLC partitioning. By using a real hardware prototype, we are able to run complete, full-sized
applications for realistic time scales on a standard operating system and accurately measure performance and energy.
...
We build our workload using a wide range of codes from three different
popular benchmark suites: SPEC CPU 2006~\cite{SPEC2006},
DaCapo~\cite{dacapo}, and PARSEC~\cite{parsec}.
We include some additional applications to broaden the
...
representative of a managed-language workloads.
We use the latest 2009 release, which consists of a set of open-source, real-world
...
multithreaded kernel representing a browser layout animation; {\tt
  G500\_csr} code is a new hybrid breadth-first search algorithm for the Graph500 contest; {\tt Paradecoder} is a parallel
...
Using the characterization of the applications, we select a subset of the benchmarks that are representative of different possible responses to resource allocations in order to reduce our study to a feasible size.
% (computational requirement grows quadratically with the number of benchmarks).  

Following in the footsteps of \cite{Phansalkar:ISCA2007}, we use
machine learning to select representative benchmarks.  We use a
hierarchical clustering algorithm~\cite{Phansalkar:ISCA2007}
provided by the Python library \texttt{scipy-cluster} with the
\textit{single-linkage} method. 
...
When adding new threads, we first assign both
hyperthreads available in one core before moving on to the next
core. For example, allocations with four threads correspond to
running on both hyperthreads of two cores. 

\subsubsection{RTF Results}

\subsubsection{Resource Allocation Decision Results}
