...
Regardless of status, columns are left in place in $R$
...
%\[
%\mu \| a_i - a_i^\mathrm{prev}\|_1,
%\]
%where $\mu > 0$ is a parameter, to the response time cost function.
...
%(re-)estimate the response time model parameters $\tau_{0,i}$, $w_i$, and
...
In our implementation, we use the simplified calculation of 
$\epsilon^\mathrm{pri}$,
...
%For example, when compared with applicationor allocation units,
