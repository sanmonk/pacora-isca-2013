...
Applications report their own measured response times to \pacora through a message-passing interface built into \tess, and \pacora uses this information to build models offile or online.  Our online models produced are identical to our offline models given the same inputs, so the method used has no effect the resource allocation decisions.  However, online models are able to adapt changes in applications. 
Modeling is discussed further in Section~\ref{RTFs}.
