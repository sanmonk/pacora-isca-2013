...
Our workload is designed to represent the scenario of a video conference with participates from many remote locations.  There is a separate, performance guaranteed incoming video stream for each participant.
...
The application adjusts video sizes based on who is speaking. The speaker's video is larger and has higher resolution than the other video streams, and as the speaker changes the requirements for the video streams change. While conferencing, network-intensive tasks such as uploading and downloading files are executed in the background.
